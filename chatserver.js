// chatserver.js (Render用・Supabase永久保存対応版)
...
const SUPABASE_KEY = 'sb_publishable_EuuRT2-EM3dYEWzKj_5fgQ_bKojR_it'; 
  ...
  ws.on('message', async (message) => {
    try {
      ...
      if (clientData.text && clientData.text.trim() !== "") {
        ...
        // 💡 【修正の核心】awaitを入れて、Supabaseへの「保存の完了」を100%確実に待つ！
        // さらにもし保存に失敗した場合はログを出して原因を追えるように強化
        const { error: insertError } = await supabase
          .from('chat_logs')
          .insert([{ text: msgData.text, sender_id: msgData.senderId }]);

        if (insertError) {
          console.error("Supabase保存失敗:", insertError);
        }

        // 保存が終わった後に、全員へメッセージを届ける
