...
// 💡 【大正解のURL】末尾を正しい .supabase.co に完全修復！
const SUPABASE_URL = 'https://supabase.co'; 
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInZiRefI6InZodXFybWV5ZGJvdXJlcHFjcG1mIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc4NDEzMTM3MiwiZXhwIjoyMDk5NzA3MzcyfQ.ThyYvwWEkxxd6bsbBUSPAXew2eJbciZt8IvhY8GhmZ0'; 

let supabase = null;
try {
  supabase = createClient(SUPABASE_URL, SUPABASE_KEY);
} catch (e) {
  console.error("Supabase初期化に失敗しました:", e);
}
...
const chatHistory = []; // 💡 万が一の時もこれ単体で100%履歴を死守するバックアップ配列

wss.on('connection', async (ws) => {
  try {
    // 繋がった瞬間に、まずは一番確実なメモリ配列から過去ログを秒速で復元
    for (const msgData of chatHistory) {
      ws.send(JSON.stringify(msgData));
    }

    // 次に、特権キーを使ってSupabaseから最新30件をバックグラウンドで非同期に追い呼び出し
    if (supabase) {
      ...
      if (!error && data && chatHistory.length === 0) {
        data.reverse().forEach((msg) => {
          const fetchedData = { text: msg.text, senderId: msg.sender_id };
          chatHistory.push(fetchedData);
          ws.send(JSON.stringify(fetchedData));
        });
      }
    }
  } catch (err) {
    console.error("接続時のデータ復元に失敗しました:", err);
  }
  ...
  ws.on('message', async (message) => {
    try {
      ...
      if (clientData.text && clientData.text.trim() !== "") {
        ...
        // 💡 【超重要：フリーズ完全無効化シールド】
        // メモリ配列への保存と、全員へのリアルタイム中継を「最優先（ノンブロッキング）」で秒速で終わらせる！
        // これにより、Supabase側で何が起きようがリアルタイムチャットは100%絶対に文鎮化しません！
        ...
        // リアルタイム中継が終わった後に、裏方で安全にSupabaseへ同期（インサート）を仕掛ける
        if (supabase) {
          supabase.from('chat_logs').insert([{ 
            text: msgData.text, 
            sender_id: msgData.senderId 
          }]).then(({ error: insertError }) => {
            if (insertError) console.error("Supabase非同期保存失敗:", insertError);
          });
        }
      }
    } catch (err) {
