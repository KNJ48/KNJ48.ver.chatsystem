// chatserver.js (Render用・Googleスプレッドシート永久保存・力技確定版)
...
// あなたの本物のスプレッドシートIDと、共有していただいた本物のGASのウェブアプリURLを焼き付け！
  ...
  ws.on('message', async (message) => {
    try {
      ...
      if (clientData.text && clientData.text.trim() !== "") {
        ...
        // 💡 【あなたの閃いた力技ハック】
        // 届いた文字がゴミ信号だったら、その場で即座に処理を中断（リターン）して完全射殺！
        // これにより、画面中継もスプレッドシートへの保存も100%絶対にされなくなります。
