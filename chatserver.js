...
// 💡 【大正解の通信URL】プロジェクトIDの前に通信専用の指定、または完全なパスを明記
...
// オプション設定を付与して、一般Webサイト（supabase.com）への誤転送を完全にブロック！
...
wss.on('connection', async (ws) => {
  try {
    if (supabase) {
      // 💡 通信先をAPIエンドポイントに強制指定して、Supabaseからデータを呼び出す
  ...
  ws.on('message', async (message) => {
    try {
      ...
      if (clientData.text && clientData.text.trim() !== "") {
        ...
        // 💡 認証オプション付きの特権キーで、裏方で安全にSupabaseへ非同期保存
        if (supabase) {
