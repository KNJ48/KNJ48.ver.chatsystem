// chatserver.js (Render用・Googleスプレッドシート永久保存対応版)
...
// 💡 あなたの本物のGoogleスプレッドシートIDを最初から完全にセット済！
...
// GASからのWebフック登録用エンドポイント
app.use(express.json());
app.post('/set-gas-url', (req, res) => {
  if (req.body.url) {
    GAS_DEPLOY_URL = req.body.url;
    console.log("GAS URL Updated:", GAS_DEPLOY_URL);
    return res.send("ok");
  }
...
wss.on('connection', async (ws) => {
  try {
    // 繋がった瞬間に、Google側から最新30件を自動読み出し
    if (chatHistory.length === 0 && GAS_DEPLOY_URL) {
    ...
    try {
      ...
      if (clientData.text && clientData.text.trim() !== "") {
        ...
        if (GAS_DEPLOY_URL) {
          ...
          }).catch(err => console.error("Google保存失敗:", err));
        }
      }
    } catch (err) {
