// chatserver.js (Render用・Googleスプレッドシート永久保存・ゴミ信号完全シャットアウト版)
...
// あなたの本物のGoogleスプレッドシートIDと、本物のGASのウェブアプリURLを完全に焼き付け！
    ...
    <body>
        ...
        <script>
            ...
            ws.onmessage = (e) => {
                ...
                // 💡 通常画面側のフロントエンドでもゴミ信号の描画をパッとスキップ
    ...
    if (chatHistory.length === 0) {
      ...
      if (res.ok) {
        ...
        if (data && Array.isArray(data)) {
          data.forEach(msg => {
            // 💡 データベース内、または読み込み時にゴミが混ざっていても確実に検閲して除外！
  ...
  ws.on('message', async (message) => {
    try {
      ...
      if (clientData.text && clientData.text.trim() !== "") {
        ...
        // 💡 【美学の検問シールド】あなたの閃いた神ハックをサーバー最深部に実装！
        // 届いたテキストが「sender_id」や「text」などのゴミキーワードだった場合は、
        // 画面の中継も、スプレッドシートへの保存も、100%全自動で「完全に無視（無効化）」して射殺する！
        ...
        // 検問を通過した純粋なチャットのみ、スプレッドシートへ安全に自動送信！
