// chatserver.js (Render用・Supabase直行永久保存完全版)
...
// 💡 【大正解への裏ルート】通信専用エンドポイント（REST API）を直撃させる！
const API_URL = 'https://supabase.co';
const SERVICE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZodXFybWV5ZGJvdXJlcHFjcG1mIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc4NDEzMTM3MiwiZXhwIjoyMDk5NzA3MzcyfQ.ThyYvwWEkxxd6bsbBUSPAXew2eJbciZt8IvhY8GhmZ0';

wss.on('connection', async (ws) => {
  try {
    // 💡 Supabaseライブラリを捨てて、標準のfetchで直接最新30件を取得！
    const res = await fetch(`${API_URL}?select=text,sender_id&order=id.desc&limit=30`, {
      headers: {
        'apikey': SERVICE_KEY,
  ...
  ws.on('message', async (message) => {
    try {
      ...
      if (clientData.text && clientData.text.trim() !== "") {
        ...
        }).catch(err => console.error("直行保存失敗:", err));
      }
    } catch (err) {
